...
static void viewscroll(const Arg *arg);
...
static int statusw;
static int elstatusw;
static int erstatusw;
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		} else if (ev->x > selmon->ww - erstatusw) {
			x = selmon->ww - erstatusw;
			click = ClkStatusText;

			char *text, *s, ch;
			statussig = 0;
			for (text = s = estextr; *s && x <= ev->x; s++) {
				if ((unsigned char)(*s) < ' ') {
					ch = *s;
					*s = '\0';
					x += TEXTW(text) - lrpad;
...
void xkbeventnotify(XEvent *e)
{
	...
	selectedKeyboardLayout[0] = xkbGlobal.group + 48; //0 ascii
	Arg arg = { .v = notifyKeyboardChange };
	spawn(&arg);
        break;
    }
}
